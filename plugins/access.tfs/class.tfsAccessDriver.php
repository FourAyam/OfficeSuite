...
class tfsAccessDriver extends AbstractAccessDriver implements AjxpWrapperProvider
{
    /**
    * @var Repository
    */
    ...
    public function switchAction($action, $httpVars, $fileVars)
    {
        ...
        $dir = $httpVars["dir"] OR "";
        if ($this->wrapperClassName == "tfsAccessWrapper") {
            $dir = tfsAccessWrapper::patchPathForBaseDir($dir);
        }
        ...
        switch ($action) {
            ...
            break;
	    //neoagerx task_assign_dialog
            case "task_assign" :
	    $loggedUser = AuthService::getLoggedUser();
	    //error_log("selectedtask".$httpVars["selectedtask"],0);	
	    //error_log("selectednode".$httpVars["selectednode"],0);	
				
	    if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"listreporters")==0){    
		    $selectednode = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectednode"]));
		    $newValues = $selectednode->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
		    $reporters = $loggedUser->getReportersByManagerid($loggedUser->getId(),0);
		    if(count($reporters)>0){
			$options="";
			foreach($reporters as $reporter) {
				if(isset($newValues["assignedto"])&&strcmp($newValues["assignedto"],$reporter)==0)
				$options.="<option selected>".$reporter."</option>";
				else
				$options.="<option>".$reporter."</option>";				
			}
		    header("Content-type:text/html");
		    print('<select id="assignto" name="assignto" multiple style="width: 200px; font-size: 12px; height: auto; margin-top:5px; margin-bottom:7px; border: 1px solid #ccc;">'.$options.'</select>');
		    }
		    else{
		    header("Content-type:text/html");
		...
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_assign")==0){
			//$tnode = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			//foreach($newValues as $key=>$value)
				//error_log("meta ".$key."=".$value,0);
			$newValues["assignedto"]=$httpVars["assignto"];
			$newValues["assignedby"]=$loggedUser->getId();
			$newValues["assignedon"]=date("Y-m-d H:i:s");
			$newValues["status"]="Assigned";
			$newValues["icon"]="task_assigned.png";
			$newValues["openicon"]="task_assigned_open.png";
			$newValues["ajxp_mime"]="tassigned";
			//$newValues["mimestring_id"]="102";
			
	 		$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_ASSIGN);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task assigned to ".$httpVars["assignto"]."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			//error_log("getDescriptionLong ".$notif->getDescriptionLong());
			$this->notificationCenter->postNotification($notif, $loggedUser->getId());
			$this->notificationCenter->postNotification($notif, $httpVars["assignto"]);
			$managers = $loggedUser->getManagerByUserId($loggedUser->getId(),0);
			if(count($managers)>0)
			foreach($managers as $manager){
				$this->notificationCenter->postNotification($notif, $manager);
			}
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
        
		}
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_unassign")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_UNASSIGN);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			$assignto = $newValues["assignedto"];
			$newValues["assignedto"]="";
			$newValues["assignedon"]="";
			$newValues["assignedby"]="";
			$newValues["status"]="Created";
			$newValues["icon"]="task.png";
			$newValues["openicon"]="task_open.png";
			$newValues["ajxp_mime"]="tcreated";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task revoked from ".$assignto."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}	    
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_accept")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$newValues["acceptedby"]=$loggedUser->getId();			
			$newValues["acceptedon"]=date("Y-m-d H:i:s");
			$newValues["status"]="Accepted";
			$newValues["icon"]="task_accepted.png";
			$newValues["openicon"]="task_accepted_open.png";
			$newValues["ajxp_mime"]="taccepted";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_ACCEPT);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			foreach($managers as $manager){
	 		...
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task accepted by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}	    
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_reject")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$newValues["rejectedby"]=$loggedUser->getId();					
			$newValues["rejectedon"]=date("Y-m-d H:i:s");
			$newValues["status"]="Rejected";
			$newValues["icon"]="task_rejected.png";
			$newValues["openicon"]="task_rejected_open.png";
			$newValues["ajxp_mime"]="trejected";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_REJECT);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			foreach($managers as $manager){
	 		...
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task rejected by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_start")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$newValues["startedby"]=$loggedUser->getId();						
			$newValues["startedon"]=date("Y-m-d H:i:s");

			$newValues["status"]="Started";
			$newValues["icon"]="task_started.gif";
			$newValues["openicon"]="task_started_open.gif";
			$newValues["ajxp_mime"]="tstarted";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_START);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			foreach($managers as $manager){
	 		...
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task started by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}	    
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_pause")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
 			$timeStart = strtotime($newValues["startedon"]);
 			$timeEnd = strtotime(date("Y-m-d H:i:s"));
			$newValues["pausedon"]=date("Y-m-d H:i:s");
			$diff = ($timeEnd - $timeStart);
			$newValues["timespent"]=(floatval($newValues["timespent"])+$diff/3600)." ManHours";			
			$newValues["pausedby"]=$loggedUser->getId();			
			$newValues["status"]="Paused";
			$newValues["icon"]="task_pau.png";
			$newValues["openicon"]="task_p.png";
			$newValues["ajxp_mime"]="tpaused";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_PAUSE);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			foreach($managers as $manager){
	 		...
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task paused by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}	    
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_assign_finish")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$timeStart = strtotime($newValues["startedon"]);
			...
			$diff = ($timeEnd - $timeStart);
			$newValues["timespent"]=(floatval($newValues["timespent"])+$diff/3600)." ManHours";			
			$newValues["finishedby"]=$loggedUser->getId();			
			$newValues["finishedon"]=date("Y-m-d H:i:s");
			$newValues["status"]="Finished";
			$newValues["icon"]="task_finished.png";
			$newValues["openicon"]="task_finished_open.png";
			$newValues["ajxp_mime"]="tfinished";
			//$newValues["mimestring_id"]="101";
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			$notif = new AJXP_Notification();
        		$notif->setAction(AJXP_NOTIF_TASK_FINISH);
        		$notif->setNode($node);
			$notif->setAuthor($loggedUser->getId());
			//error_log("getDescriptionShort ".$notif->getDescriptionShort());
			//error_log("getDescriptionBlock ".$notif->getDescriptionBlock());
			...
			foreach($managers as $manager){
	 		...
	 		AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task completed by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("UPDATE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}	    
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_hide")==0){
			//$node = $selection->getUniqueNode($this);
			$node = new AJXP_Node($this->urlBase.AJXP_Utils::decodeSecureMagic($httpVars["selectedtask"]));
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$newValues["hiddenfrom"]="";			
			
			if(isset($newValues["hiddenfrom"])){
				$hiddenfrom=explode(',', $newValues["hiddenfrom"]);
				$hiddenfrom[]=$loggedUser->getId();
				$newValues["hiddenfrom"]=implode(',', $hiddenfrom);
			}else{
				$hiddenfrom =array($loggedUser->getId());
				$newValues["hiddenfrom"]=implode(',', $hiddenfrom);
			}
			
			$node->setMetadata("users_meta", $newValues, false, AJXP_METADATA_SCOPE_GLOBAL);
			AJXP_XMLWriter::header();
			AJXP_XMLWriter::write("<message type='SUCCESS'>Task hidden by ".$loggedUser->getId()."</message>", true);
			AJXP_XMLWriter::writeNodesDiff(array("REMOVE" => array($node->getPath() => $node)), true);
			AJXP_XMLWriter::close();
			
			//AJXP_Controller::applyHook("msg.instant",array("<reload_user_feed/>",AJXP_REPO_SCOPE_ALL,$notif->getTarget()));
		}

		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_listhidden")==0)
		{
		//error_log("task unhide called",0);
		$dir = "";
		$path = $this->urlBase.($dir!= ""?($dir[0]=="/"?"":"/").$dir:"");
		$nodes = scandir($path, 1);
		//error_log("nodes count ".count($nodes),0);
		header("Content-type:text/html");
		print('<select id="unhidenodes" name="assignto" multiple style="width: 200px; font-size: 12px; height: auto; margin-top:5px; margin-bottom:7px; border: 1px solid #ccc;">');
		...
		foreach ($nodes as $nodeName) {
			...
			$node = new AJXP_Node($path."/".$nodeName);
			$newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
			$hiddennodes = array();
			if(isset($newValues["hiddenfrom"])){
					$hiddenfrom = explode(",",$newValues["hiddenfrom"]);
					if(count($hiddenfrom)>0){
						foreach($hiddenfrom as $hidefrom){
							if(strcmp($hidefrom,$loggedUser->getId())==0)
								print('<option value="'.$node->getPath().'">'.$nodeName.'</option>');
						}
					}
				}
			}
		...
		if(isset($httpVars["sub_action"])&&strcmp($httpVars["sub_action"],"task_unhide")==0)
		{
			error_log("unhidetaskcalled",0);
			error_log("unhidetask".$httpVars["selectedtask"],0);
		}
	    break;
		...
		//neoagerx		
		if(strcmp(dirname($newNode->getPath()),"/")==0){
                	$messtmp.="$mess[518] ".SystemTextEncoding::toUTF8($dirname)." $mess[39] ";
		}else{
		...
		$metadata = $newNode->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
        	$metadata["createdby"] = $user->getId();
	        $metadata["createdon"] = date("Y-m-d H:i:s");
		$metadata["status"]="Created";
		$metadata["timespent"]="0";
	      	if(strcmp(dirname($newNode->getPath()),"/")==0){
			$metadata["icon"]="task.png";
			$metadata["openicon"]="task_open.png";
			$metadata["ajxp_mime"]="tcreated";
		}
			$newNode->setMetadata("users_meta", $metadata, false, AJXP_METADATA_SCOPE_GLOBAL);
		
                //error_log("loggedin".$user->getId(),0);
		//error_log("findinf user's manager ".$user->getId(),0);
			$this->watcher->setWatchOnFolder($newNode,$user->getId(),MetaWatchRegister::$META_WATCH_CHANGE);
			$managers = $user->getManagerByUserId($user->getId(),0);
		//error_log("COUNT user's manager =".count($managers),0);		
			if(count($managers)>0){
				foreach($managers as $manager){
					//error_log("user's manager ".$manager,0);
					$this->watcher->setWatchOnFolder($newNode,$manager,MetaWatchRegister::$META_WATCH_CHANGE);
				}	
               		}
		if(!isSet($nodesDiffs)) $nodesDiffs = $this->getNodesDiffArray();
            ...
            case "ls":
                ...
                $nonPatchedPath = $path;
                if ($this->wrapperClassName == "tfsAccessWrapper") {
                    $nonPatchedPath = tfsAccessWrapper::unPatchPathForBaseDir($path);
                }
		...
		$loggedUser = AuthService::getLoggedUser();                
		    ...
		    //neoagerx
		    $newValues = $node->retrieveMetadata("users_meta", false, AJXP_METADATA_SCOPE_GLOBAL);
		    //error_log("ls node".$node->getPath(),0);
		    //error_log("ls logged".$loggedUser->getId(),0);
		    //$newValue =  $node->metadata;
		    //foreach($newValues as $key=>$value){
			//error_log("ls".$key."=".$value,0);}
		    $reporters = $loggedUser->getReportersByManagerId($loggedUser->getId(),0);
		    $show = false;
		 	//if(!in_array($loggedUser->getId(),$hiddenfrom))
				//$show = true;
			//}
		    if(isset($newValues["createdby"]))
			if(in_array($newValues["createdby"],$reporters)||strcmp($newValues["createdby"],$loggedUser->getId())==0)
				$show = true;
		    if(isset($newValues["assignedto"]))
			if(strcmp($newValues["assignedto"],$loggedUser->getId())==0)
				$show = true;
		    $hide = false;
		    if(isset($newValues["hiddenfrom"])){
			$hiddenfrom = explode(",",$newValues["hiddenfrom"]);
			...
			foreach($hiddenfrom as $hidefrom)
				if(strcmp($hidefrom,$loggedUser->getId())==0){
				//error_log("hidden from".$hidefrom,0);
				$hide = true;}
			}
    ...
    public function readFile($filePathOrData, $headerType="plain", $localName="", $data=false, $gzip=null, $realfileSystem=false, $byteOffset=-1, $byteLength=-1)
    {
        ...
        if (!$realfileSystem && $this->wrapperClassName == "tfsAccessWrapper") {
            $originalFilePath = $filePathOrData;
            $filePathOrData = tfsAccessWrapper::patchPathForBaseDir($filePathOrData);
        }
        session_write_close();
            ...
            */
            ...
            if ( isset($_SERVER['HTTP_RANGE']) && $isFile && $size != 0 ) {
                if ($headerType == "stream_content") {
                    if (extension_loaded('fileinfo')  && $this->wrapperClassName == "tfsAccessWrapper") {
        ...
        if ($data) {
            ...
            if ($this->getFilteredOption("USE_XSENDFILE", $this->repository->getId()) && $this->wrapperClassName == "tfsAccessWrapper") {
                if(!$realfileSystem) $filePathOrData = tfsAccessWrapper::getRealFSReference($filePathOrData);
    ...
    if ($this->getFilteredOption("USE_XACCELREDIRECT", $this->repository->getId()) && $this->wrapperClassName == "tfsAccessWrapper" && array_key_exists("X-Accel-Mapping",$_SERVER)) {
        if(!$realfileSystem) $filePathOrData = tfsAccessWrapper::getRealFSReference($filePathOrData);
        $filePathOrData = str_replace("\\", "/", $filePathOrData);
	...
	//{
	//error_log("metadata ".$key."=".$value,0);
	//}
    ...
    public function createEmptyFile($crtDir, $newFileName, $content = "")
    {
        if (($content == "") && preg_match("/\.html$/",$newFileName)||preg_match("/\.htm$/",$newFileName)) {
            $content = "<html>\n<head>\n<title>New Document - Created By Office Suite</title>\n<meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\">\n</head>\n<body bgcolor=\"#FFFFFF\" text=\"#000000\">\n\n</body>\n</html>\n";
