 ...
 * @subpackage Access
 */
class tfsAccessWrapper implements AjxpWrapper
{
    /**
     * FileHandle resource
     *
            ...
            //print($streamType.$path);
               if ($streamType == "file") {
                   if (self::$crtZip == null ||  !is_array(self::$currentListingKeys)) {
                       ...
                       register_shutdown_function(array("tfsAccessWrapper", "removeTmpFile"), $tmpDir, $tmpFileName);
    ...
    public function stream_stat()
    {
        ...
        if (is_resource($this->fp)) {
            ...
            tfsAccessWrapper::$lastRealSize = false;
            if ($statValue[2] > 0 && $PROBE_REAL_SIZE && !ini_get("safe_mode")) {
                tfsAccessWrapper::$lastRealSize = floatval(trim($this->getTrueSizeOnFileSystem($this->realPath)));
            }
            return $statValue;
        }
