...
class sqlConfDriver extends AbstractConfDriver
{
    ...
    //Fourayam
    public function getReportersByManagerid( $userId ,$opt)
    {    
        $reporters = array();
        $reporter_results = dibi::query('SELECT [ajxp_user_manager].[login] FROM [ajxp_user_manager] WHERE [manager] = %s', $userId);
	$all = $reporter_results->fetchAll();
        foreach ($all as $item) {
		if($opt==1)
       		     $reporters[] = $this->createUserObject($item["login"]);
		else
       		     $reporters[] = $item["login"];
        }
	
        return $reporters;
    }
    public function getManagerByUserId( $userId ,$opt)
    {    
        $managers = array();
        $manager_results = dibi::query('SELECT [ajxp_user_manager].[manager] FROM [ajxp_user_manager] WHERE [login] = %s', $userId);
	$all = $manager_results->fetchAll();
        foreach ($all as $item) {
		if($opt==1)
       		     $managers[] = $this->createUserObject($item["manager"]);
		else
       		     $managers[] = $item["manager"];
        }
        return $managers;
    }
    public function addManagerByUserid( $userId ,$manager)
    {    
        $manager_results = dibi::query('INSERT INTO [ajxp_user_manager] ([login],[manager]) VALUES (%s,%s)', $userId,$manager);
	return $manager_results;
    }
    public function removeManagerByUserid( $userId ,$manager)
    {    
        //$manager_results = dibi::query('DELETE FROM [ajxp_user_manager] WHERE [login] = %s AND [manager] = %s', $userId,$manager);
	$manager_results = dibi::query('DELETE FROM [ajxp_user_manager] WHERE [login] = %s', $userId);
	error_log("delete action : DELETE FROM [ajxp_user_manager] WHERE [login] =".$userId,0);
	return $manager_results;
    }
