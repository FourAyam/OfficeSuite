...
class AJXP_NotificationCenter extends AJXP_Plugin
{
    ...
    public function loadUserFeed($actionName, $httpVars, $fileVars)
    {
        ...
        foreach ($res as $n => $object) {
		    ...
		    //error_log("accesstype".$node->getRepository()->getAccessType(),0);
		    if($node->getRepository()->getAccessType()=="tfs"){
			    $managers = $u->getManagerByUserId($notif->getAuthor(),0);
			    if (!in_array($userId, $managers)&&$notif->getAuthor()!=$userId) {
				continue;
			    }
		    }		    
