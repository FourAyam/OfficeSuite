...
Class.create("RoleEditor", AbstractEditor, {
    ...
    //fourayam
    updateManagers : function(selection){
        // DIFF
        var orig = this.roleData.USER.MANAGER || $A();
        var currentUserId = this.roleId.replace("AJXP_USR_/", "");
        orig.each(function(el){
            if(!selection[el]) {
                var conn = new Connexion();
                conn.setParameters({
                    get_action:"edit",
                    sub_action:"user_delete_manager",
                    user_id : currentUserId,
                    manager : el
                });
                conn.sendSync();
    ...
    buildInfoPane : function(node, scope){
	    ...
	    //FourAyam
            var managerChoices = this.roleData.ALL.MANAGERS;
	    var mindex = managerChoices.indexOf(this.roleId.replace("AJXP_USR_/", ""));
	    //alert(this.roleData.ALL.MANAGERS);
 	    managerChoices.splice(mindex, 1);
	    managerChoices= managerChoices.join(",");
	    ...
	    var repos = [];
            ...
            var defs = [
                ...
                $H({"name":"managers",label:MessageHash["ajxp_role_editor.43"],"type":"select", multiple:true, choices:managerChoices, default:this.roleData.USER.MANAGER.join(",")})
	    ...
	    //FourAyam
	    var managerSelect = this.element.down("#pane-infos").down("#account_infos").down('select[name="managers"]');
            managerSelect.observe("change", function(){
                this.setDirty();
                if(this.updateManagerAccumulator){
                    window.clearTimeout(this.updateManagerAccumulator);
                }
                this.updateManagerAccumulator = window.setTimeout(function(){
                    this.updateManagers(managerSelect.getValue());
                }.bind(this) , 500);
            }.bind(this) );
